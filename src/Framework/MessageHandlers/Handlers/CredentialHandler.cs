				...
				case nameof(CredentialOfferRequest):
					{
						...
						return Any.Pack(await CredentialOffer(offerRequest, context), Constants.MessagePrefix);
					}
				case nameof(CredentialRequest):
					{
						var credentialRequest = msg.Unpack<CredentialRequest>();
						return Any.Pack(await Credential(credentialRequest, context), Constants.MessagePrefix);
					}
			}
